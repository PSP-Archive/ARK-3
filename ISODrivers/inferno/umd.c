...
int checkUserPtr(const void *addr, int size);
...
int sceUmdGetDiscInfo(pspUmdInfo *info)
{
	...
	if(!checkUserPtr(info, sizeof(*info)))
	{
		ret = 0x80010016;
		goto exit;
	}
...
int sceUmdMan_driver_4FFAB8DA(u32 a0, u32 a1, u32 a2)
{
	...
	_sw(intr, text_addr + ISOFS_WAIT_SEMA_CALL);
	_sw(intr, text_addr + ISOFS_SIGNAL_SEMA_CALL_1);
	_sw(intr, text_addr + ISOFS_SIGNAL_SEMA_CALL_2);
	_sw(intr, text_addr + ISOFS_SIGNAL_SEMA_CALL_3);
...
int sceUmdActivate(int unit, const char* drive)
{
	...
	if(drive == NULL || !checkUserPtr(drive, strlen(drive) + 1))
	{
		return 0x80010016;
	}
	...
	UNUSED(err);
	err = sceIoAssign(drive, "umd0:", "isofs0:", 1, &value, sizeof(value));
	printk("%s: sceIoAssign -> 0x%08X\r\n", __func__, err);
	sceUmdSetDriveStatus(PSP_UMD_PRESENT | PSP_UMD_INITED | PSP_UMD_READY);
	printk("%s: sceUmdSetDriveStatus\r\n", __func__);

	printk("%s: g_game_fix_type 0x%08X\r\n", __func__, g_game_fix_type);
...
int powerEvtHandler(int ev_id, char *ev_name, void *param, int *result)
{
