...
#define CONF_ADDR 0x8BFFFF00
typedef struct ARKConfig{
	char savepath[SAVE_PATH_SIZE];
	char gameid[9];
	char can_install_game;
	char is_vita_pops;
	char can_write_eboot;
} ARKConfig;
#define ark_config ((ARKConfig*)CONF_ADDR)
#define SAVEPATH (ark_config->savepath)
#define GAMEID (ark_config->gameid)
#define NEWER_FIRMWARE !(ark_config->can_install_game)
#define IS_VITA_POPS (ark_config->is_vita_pops)
#define CAN_WRITE_EBOOT (ark_config->can_write_eboot)
