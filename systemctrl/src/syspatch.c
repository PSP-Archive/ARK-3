...
void vitaPopsOnModuleStart(SceModule2* mod){
	...
	PRTSTR(mod->modname);
	copyPSPVram(NULL);
	u32 mod1 = sceKernelFindModuleByName(mod->modname);
	u32 mod2 = sceKernelFindModuleByUID(mod->modid);
	printk("%s:%d:%d\n", mod->modname, mod1, mod2);
	#endif
...
*/
	...
	if(NEWER_FIRMWARE && 0 == strcmp(mod->modname, "sceKermitPeripheral_Driver"))
	{
		// Patch Kermit Peripheral Module
		patchKermitPeripheral(mod);
		// Exit Handler
