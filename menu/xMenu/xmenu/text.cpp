...
void TextAnim::draw(float y){
	...
	intraFontSetStyle(common::getFont(), 0.7f, LITEGRAY, (0xFF<<24)+(val<<16)+(val<<8)+(val), 0.0,0);
	intraFontPrint(common::getFont(), 200, y+30, this->glowText.c_str());

	intraFontSetStyle(common::getFont(), 0.5f, WHITE, DARKGRAY, 0.0, INTRAFONT_SCROLL_LEFT);
