...
void flashVitaDump( char *packName )
{
	...
	if (packFileID)
	{
		...
		while ( f0[flash0_fileindex].name != NULL )
		{
			...
			if ( name[strlen(name)-3] == 'p' && name[strlen(name)-2] == 'r' && name[strlen(name)-1] == 'x' )
			{
				switch ( *((int*)f0[flash0_fileindex].content) )
				{
					...
					default:
						//PRTSTR("Unknown prx : %08lX", (u32)name);
						isValid = 1;
						break;
				}
			}
			...
			{
				////PRTSTR("Invalid flash file : %08lX", name);
				//PRTSTR("Dummied flash file");
			}
		...
		while ( *curf0Backup != 0 )
		{
			...
			{
				//PRTSTR("%08lX not found", origContent);
		...
		////PRTSTR("Dummy files: %08lX, backup files: %08lX ", dummy_count, backup_count);
		//setPrintColorRGB(255, 0, 0);
		//PRTSTR("");
		//PRTSTR("Flash successfully dumped");
		//PRTSTR("");
		//setPrintColorRGB(0, 255, 255);
	}
}
