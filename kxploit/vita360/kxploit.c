...
void repairInstruction() {
	...
	PRTSTR("Going back to kernel stuff.");
}
...
int stubScanner(void){
    ...
    _sceNpCore_8AFAB4A0 = g_tbl->FindImportUserRam("sceNpCore", 0x8AFAB4A0);
...
void executeKernel(u32 kernelContentFunction){
  ...
  _sw(NOP, 0x0888888C);
  g_tbl->KernelDcacheWritebackAll();
  //PRTSTR("Executing Kernel Exploit...");

  PRTSTR1("g_tbl->KernelLibcClock: %p", &g_tbl->KernelLibcClock);
